...
- **Closed gap**: a plugin that is already fully compatible never re-enters setup, so setup alone left its dispatcher at whatever version patched it. That was survivable while a patched hook was a self-contained bash script, and fatal once a hook became a descriptor: `wrapper_missing` repair writes the entry and deletes the legacy wrapper file, while a dispatcher from before the descriptor still execs that deleted file — the hook dies silently, exits 0, and `status` still reads healthy. `verify` therefore requires the dispatcher to be **current**, not merely present, comparing each `DISPATCHER_FILES` entry against the shipped template and reporting the same `cmd_missing` type with `is stale` instead of `not found`. It runs before the entry repairs so the plugin is never dead in between, and `templateCmd` reaches the read-only `status` path too, or the machine this was found on would still look healthy.
